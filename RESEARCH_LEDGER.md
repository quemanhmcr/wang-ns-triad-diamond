...
This still does **not** make `D_V` a finite reset or a new-new heat edge into a selected transfer parent.  The high-strain route now has simultaneous frequency ancestry, spatial coherent service, and exact material endpoint ownership.  Its remaining seam is quantitative: OO service needs a heat-valid reservoir history/capacity theorem, ON service must be attached to actual interface/relink work rather than merely named, and NN service must produce ancestry/service without a packet-mass floor.  Fixed-shell/material-chart commutators must remain explicit.
