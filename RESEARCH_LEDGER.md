...
Status: current through the certified **material-sidecar inherited-stock central/joint routing** theorem, exact theorem SHA `376fd52e1c891c774b70cf339b192bea7045107f`. This file is deliberately theorem-facing and compact. CI runs, stress counts, residuals, artifact digests, and failure lineage live in `docs/experimental_certification_log.md`; the pre-compaction full ledger is archived under `docs/history/`.
...
- Central master wiring realizes the inherited-stock quotient for sidecar-free certificates and now also for **sidecar-bearing certificates with the exact Phase-A decomposition**.  Master stock mass remains bound to the same `E0`; the stock-relay label is forbidden as a recursive owner; membership provenance and selected-family `R_switch` are carried separately beside the one stock charge.  Untyped or un-decomposed sidecar-bearing inheritance remains event-facing fail-closed.
- Sidecar-bearing inherited stock is now typed **before** central integration.  Intrinsic material-membership rereading is zero-charge provenance because it only repartitions fixed positive service.  A selected-family change carries the exact Moyal boundary charge `R_switch=sum_{C in S_old Δ S_new}E_C`; inherited stock remains one charge and `R_switch` is never identified with `dW`, stock `E0`, a second coefficient impulse, or smooth `K_phys`.  The exact same-state anti-theorem has `R_switch>0` with every cell increment, `P+`, `P-`, and total state change equal to zero, so the selected-family boundary charge itself has zero generation depth.  Genuine material/source recursion therefore requires separate physical service/source evidence.
- Central/joint routing now consumes that typing without changing the certified core joint-stop law.  A typed wrapper runs the physical/internal first-stop projection **before** attaching material sidecars, so positive `R_switch` alone cannot create a stop, alter first time, join a heterogeneous physical tie, or change disposition.  Conversely an independently witnessed `PhysicalCause.MATERIAL_RELINK` remains an ordinary recursive physical hit even when an `R_switch` sidecar is present at the same observation.  Exact source/strain/material ties remain unsplit.

## 6. Current frontier

1. **Genuine material/source-service recurrence:** the inherited-stock/material-boundary seam is now closed through central and joint routing.  What remains under the material owner is therefore not selected-family bookkeeping: any recursive `MATERIAL_RELINK` / new-coherent-ancestry branch must be backed by an **independently witnessed physical service/source event**.  The next material theorem should descend into that native service mechanism and separate any same-event conservative ancestry from genuinely generated service, without reusing the Moyal `R_switch` boundary currency as physical work.

**Handoff:** sidecar-bearing inherited stock is fully routed: one stock charge relays, membership is zero-charge provenance, selected-family `R_switch` is a non-event boundary sidecar, and independently witnessed material service remains recursive.  Continue from the native physical material/source-service recurrence itself, not from observer-family switching.
...
`physical_energy_causal_bridge.md` → `high_frequency_dissipation_reentry.md` → `helical_physical_edge_registration.md` → `continuum_helical_edge_measure_registration.md` → `canonical_positive_edge_work_routing.md` → `cyclic_helical_triad_donor_kernel.md` → `helical_mode_set_energy_continuity.md` → `radial_spectral_crossing_layer_cake.md` → `hard_tail_true_upward_supply.md` → `resolved_contact_native_binding.md` → `pure_uv_true_upward_natural_window.md` → `same_carrier_inherited_energy_relay.md` → `material_sidecar_stock_owner_decomposition.md` → `material_sidecar_stock_central_routing.md` → `master_no_escape.md`.
