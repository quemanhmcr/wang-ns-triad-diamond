...
The canonical positive-edge routing optimization is the motivating example.  Its original single-process stress took `62.50 s` for `1,000` samples with peak RSS near `40 MB`.  Profiling exposed redundant construction rather than expensive theorem algebra: each sample rebuilt the same three physical helical geometries and their eight-sector registrations although only three positive quotient-measure weights and `tau` varied, and one verified ledger was replayed more than once inside the routing transaction.  Pre-registering the immutable physical fibers, rescaling only their measure masses, caching immutable atom properties, and eliminating the redundant internal replay reduced `1,000` samples to `2.55 s`, a measured wall-time reduction of about `95.9%`.  The same optimized one-core path completed `50,000` samples in `117.83 s` with peak RSS about `39.5 MB`.  Before treating this as a valid engineering replacement, `64` matched RNG cases were evaluated through both full re-registration and optimized rescaling paths; the fibers, continuum ledger, exact role map, routing result and deliberate coarse compression agreed bit-for-bit at the dataclass level.  This benchmark is engineering evidence only, but it records the central lesson: a slow experiment may be dominated by recomputation of mathematical invariants rather than by the theorem being tested.
