...
The next continuum simplification suggested by the same dyadic algebra is generic rather than high-strain-specific.  If any already-certified route supplies a normalized dissipation lower `D_V>=D_0>0` on a natural block, then setting `mu_0=D_0/c` makes the low-critical-mass portion cost at most `D_0/2`; consequently at least half the actual `D_V` mass lies on shells with `M_j||P_j u||_2^2>=D_0/c`.  Such an atom would give renewed shell mass `A||P_j u||_2^2>=3D_0/(4c)` and, after the same no-hit/analysis-dual/annular-heat chain, a positive own-scale service threshold proportional to `D_0`.  This generic `D_0` re-entry is a **current theorem target**, not yet canonical or certified.

This matters because the existing source episode theorems already return integrated physical outputs rather than merely pointwise hints: SGS source weight routes to high-frequency normalized dissipation or low/base mass/entropy/cycle service, while the viscous source gives `D_V>=b Sigma_*^2/(4c)` by Cauchy without a temporal-persistence hypothesis.  The source frontier is therefore increasingly a finite routing/continuation assembly problem, not evidence for a new artificial packet selector.  Genuine temporal material relink remains a separate recursive route: same-time spatial ON service is already interface/old-incident physics, whereas a true selected-set switch has the exact Moyal symmetric-difference energy ledger and must still be connected to one of the certified renewed-service/cost/ancestry exits without inventing work or whole-shell mass.

The decisive remaining continuum bridge is therefore **universal physical renewal/exhaustion for the remaining recursive source/SGS and genuine-relink routes, together with the final master assembly**.  Critical high-strain dissipation itself now has a direct continuum entrance to named stop, initial boundary or own-scale coherent service.  Outer moving roles, event-role registration, nonaffine-interface provenance, physical pair weights, common-slice survival algebra, exact-tie projection, local measurable first-hit extraction, smooth material-carrier relay, resolved-cutoff scale renewal, dissipation-weighted resolved ancestry, bounded annular service re-entry and the high-strain short route are no longer unresolved bridges.  Consequently there is still no Navier--Stokes global-regularity proof.
