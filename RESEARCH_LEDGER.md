...
A second simplification is now strongly indicated but **not yet promoted to theorem**.  The smooth carrier state `(Q_Au,psi,z)` and its Duhamel identity contain no old-pool membership variable; current `material_relink` in common-slice registration is an external stop-side condition.  This suggests quotienting a **pure intrinsic-material label change** from carrier continuation: if only the zeta/ancestry label changes while `Q_A` and the analysis probe do not, there is no coefficient impulse to kill the carrier, and material ownership can simply be reread from the next actual service law.  A genuine role/probe change must still enter the existing interface/relink ledger.  Repeated ancestry changes are therefore not declared free; the proposed quotient would remove only a non-PDE carrier obstruction, not the master ancestry charge.

The decisive remaining continuum bridge is consequently narrower again: **assemble the remaining source/pressure/reservoir suppliers and prove the material-label/physical-role relink quotient needed for universal renewal, then close the continuum master**.  High strain, generic resolved dissipation and dominant fresh coherent mass already share one critical-shell-to-service re-entry theorem.  No Navier--Stokes global-regularity proof is claimed.
