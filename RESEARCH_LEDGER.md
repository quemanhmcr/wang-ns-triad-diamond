...
This does **not** make `D_V` a finite reset and does not yet make a global critical shell or coherent heat edge into a selected transfer parent.  The high-strain route now has simultaneous frequency ancestry and spatial coherent service, but material ownership/renewed-slab attachment remains: the seed must attach to a transported material reservoir/carrier with explicit commutator/interface control, or failure of attachment must itself be a named source/relink event.
