...
The continuum frontier is therefore narrower again.  The anonymous high-frequency-enstrophy exit is gone: it now becomes inherited critical shell or actual positive regeneration/interface work.  The main pressure inefficiency is now visibly **self-inflicted by coarse graining**: the current compiler first replaces the resolved pressure Hessian by aggregate low-pass mass `mu_V`, even though the true elliptic source is bilinear and each low--low pair carries the strong derivative factor `(M_max/N)^4`.  The next native theorem should scalarize the actual averaged pressure-Hessian tensor by its event dual, decompose the resolved `V tensor V` contribution exactly into hard dyadic shell pairs, use positive pair atoms as the pressure source law, and attach coherent/material labels only as sidecars.  Dominant pair service should then expose derivative-weighted critical reservoir mass; diffuse pair service should pay pair/component entropy; fixed material reuse has the certified one-fifth lifetime above.  This would remove aggregate `mu_V` from the canonical pressure route rather than building a synchronization packet around it.  This direct pair atomization is the active research direction and is not yet theorem-level.  Final continuum master topology, regeneration-work continuation and supplier-specific signed-good scale geometry remain open.  No Navier--Stokes global-regularity proof is claimed.
