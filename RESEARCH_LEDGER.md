...
The HH complement remains distinct from pure-UV support, but both physical parents are `<=5M/4`. Because `0<=S<=1` also gives `||h||_2<=||u||_2`, its absolute Young natural-window capacity improves by factor four relative to the old `|S|<=1` complement bound. It can enter sliding natural time directly with `R=5/4`, without aggregate shell re-Hahn or an output-scale locality theorem.
