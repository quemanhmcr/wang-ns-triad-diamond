...
The stress alternative is exactly an effective SGS source weight `Sigma_P/2` and enters the same coherent-service route.  The resolved low-pass mass alternative is **not** promoted to a generic critical shell.  It remains pressure-reservoir occupation.  On a supplied signed-good low-strain lineage, each fixed materially reused low-low pair has pressure-service ratio `194481/655360<1/3`, so persistent pressure service must relink pairs, fragment into component entropy/cycle, leave low strain, or use the SGS branch.
