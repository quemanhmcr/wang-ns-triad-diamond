# Recursive generated physical-work first-stop constructor

Status: **EXACT_GENERATED_PHYSICAL_FIRST_STOP_SURVIVAL__JOINT_MASTER_UNSPLIT__LOCAL_FIRST_HIT_AND_GENERATED_SURVIVOR_RELAY_SUPPLIED__UNIVERSAL_SLAB_RENEWAL_REMAINS**.
...
Stress: `50000` random physical-work measures
- worst total-mass reconstruction residual: `6.495e-16`
- minimum positive survivor productivity: `3.294e-08`
- maximum exact-tie event fraction: `1.000`
- majority-continuation samples: `3659`
- majority-stopped/regenerated samples: `46341`
- zero-survivor samples: `12632`

This closes the **measure-level survival/first-stop algebra** of the generated branch without requiring exact-tie fractions.  The companion smooth-SGS theorem now supplies the measurable local first-hit cause set and proves local no-hit exhaustion once a block is supplied.  The companion smooth material-carrier and cutoff-repartition theorems now remove **hard-event reselection at the common slice**: the same smooth carrier is re-anchored losslessly, its registered coefficient gives carrier energy, and changing `S_(N/4)u` to the parent cutoff is an exact repartition of the same `-Q B(u,u)` interaction.  Hard roles are read only when actual HH work occurs.  The remaining continuum bridge is universal physical slab renewal/exhaustion for every recursive source/dissipation/relink route; no Navier--Stokes global-regularity conclusion is claimed.
