...
def test_evolved_actual_galerkin_ns_preserves_cyclic_donor_kernel_and_signed_good_side_recipient():
    ...
    run = out.runs[0]
    assert run.worst_cyclic_energy_conservation_native_residual < 5e-10
    assert run.worst_cyclic_coupling_native_residual < 5e-12
    assert run.worst_measure_donor_marginal_native_residual < 5e-10
    assert run.worst_measure_recipient_marginal_native_residual < 5e-10
