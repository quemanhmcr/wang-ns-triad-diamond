...
### 6.1 One Poisson energy profile contains the whole critical hierarchy

The probability reading above is still not the lowest object.  Put

\[
\boxed{
\mathscr E(y,t):=\|P_yu(t)\|_2^2,
\qquad
P_y:=e^{-y\Lambda},
\qquad y\ge0.}
\]
...
For the **critical escape question**, the lowest object is now the canonical Poisson energy profile

\[
\boxed{
\mathscr E(y,t)=\|e^{-y\Lambda}u(t)\|_2^2.}
\]

Its first four boundary jets are `E,K,Z,M3`, and full NS is the single depth equation
...
Thus Euler is invisible at zero depth, enters criticality only through the canonical failure of
Poisson multiplicativity, and viscosity is nonpositive at every depth.  The energy and critical laws
are consecutive normal derivatives of this same equation.
...
negative heat are not enough.  The remaining theorem must use the **physical realizability** of
`D_y`, the Poisson product defect generated by the same `u` and `omega`.  The spacetime Hom--Bianchi
law remains an exact compatibility reading of that same current, but does not supply a positive
budget by itself.

In the primitive grammar: **Euler can only redistribute what failure of Poisson multiplicativity
creates; heat only erases.  Critical escape would require infinite normalized boundary action from
that physical reversible defect.**  Proving that this action is finite on every finite smooth
history is the remaining theorem.  It is **not proved**, and no global-regularity claim is made.
