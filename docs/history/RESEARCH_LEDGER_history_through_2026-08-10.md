> **Historical note.** This file is the frozen chronological research ledger through the certified high-tail sliding natural-window reentry theorem on 2026-08-10. It intentionally preserves exploratory paths, failed fixtures, numerical provenance, superseded formulations, and the order in which results were discovered. It is no longer the current compact summary of the programme. For the present canonical architecture, read [`RESEARCH_LEDGER.md`](../../RESEARCH_LEDGER.md).
...
Thus coherent-cell quarter dominance is no longer needed for **fresh-service renewal entrance** at theorem level.  The objective-SGS compiler migration is now theorem-level as well.  The old coherent-cell realization is retained only as a fine ancestry sideledger: its fresh-cluster / atomic-entropy / ancestry-entropy / same-ancestry-cycle outputs are explicitly marked `canonical_renewal_fate=False` and `SIDELEDGER_ONLY`.  They cannot alter the master-facing PDE owner.

The canonical realized SGS interface consumes the actual low-service data and the refinement-invariant fresh band law `(F_j)`.  High-frequency dissipation, uneroded old-pool capacity, selected-interface `Xi`, and fresh-scale shell conditions are read **independently from the same physical law**.  Every satisfied physical owner is retained jointly; source-code branch order has no theorem meaning.  In particular, on the fresh owner the compiler calls the certified scale theorem and records

`mu_hard exp(H_inf^scale) >= Y/(24c)`,

with no coherent-cell argmax.  `H_inf^scale` and `H_2^scale` remain deterministic service-scale concentration coordinates, never child-energy causal probabilities or stop classes.  Coherent-cell entropy/cycle remains optional ancestry metadata only.  Exact simultaneous high-tail / old-capacity / `Xi` / fresh-scale hits remain one joint native owner set rather than being broken by lexicographic priority.

The initial compiler SHA `3e93191` had `617` passing tests and exactly one fixture-only failure: the existing regression expected the literal phrase `pressure H2 is not a causal child-energy probability`, while the new certificate had combined pressure and fresh-scale warnings in one sentence.  No theorem inequality or stress step failed.  Commit `5461bb45119a709179c8a1dfd2c5ba6a0ffe3240` restored only that certificate wording.  Dedicated source-compiler run `31347901411` on exact `5461bb4` then passed the full pytest suite, `50,000` compiler states, and direct high-tail, fresh-scale, critical-shell, pressure-pair, pressure-reuse and material-sidecar dependencies.  The compiler stress reported minimum owner-pigeonhole margin `1.796e-03`, pressure diagnostic split margin `-2.220e-16`, pressure entropy-shell margin `-4.547e-13`, pressure full-survivor service margin `-4.441e-16`, fresh-scale shell margin `2.403e-05`, fresh-scale full-survivor service-conjugacy margin `1.966e-18`, maximum sampled joint source-owner count `4`, and **zero coherent-cell argmax regressions**.  High-frequency workflow `31347901424` and full physical-energy causal integration `31347901409` both passed on the same exact SHA; the latter passed Rényi, branch compiler and master episode stress.  Stored artifact: `recorded-results/31347901411/`.
...
This closes the question **what is the physical cause of high-tail regeneration?** without solving a different question by assumption.  The next Fourier-scale question is now theorem-level too: generic HH parents may be genuinely nonlocal, but the amount of such ultraviolet work is paid directly by physical tail dissipation rather than by a packet count or a chosen locality graph.

Read locality from the **actual positive aggregate HH output-shell law before coherent Hahn refinement**:

`H_j = N int [r_HH,j(t)]_+ dt`, `H=sum_j H_j >= nu D_tail/2`.

Normalize `p_j=H_j/H`, choose the maximal hard output shell `M=2^jN`, and set `H_inf^out=-log p_max`.  Atomic positive Hahn mass is explicitly forbidden as a substitute for `H_j`, because cancellation may make the former strictly larger and thereby manufacture a different scale law.

For any field supported above `K`, exact Fourier Holder gives

`||fhat||_(3/2) <= (4pi/3)^(1/6) K^(-1/2)||grad f||_2`.

If one parent feeding a child `|xi|<=M` lies above `R M`, triad closure forces the other above `(R-1)M`.  The strict transporter `S_(M/4)u` vanishes on these ultraviolet parent frequencies, so the unresolved field equals `u` there **exactly**; no cutoff multiplier loss or new `Xi` charge appears.  Combining the high-pass bounds with the physical Young coefficient and the child hard-shell mass estimate collapses all geometric constants to

`C_Y C_hp^3 = 3 sqrt(pi)`.

Hence, if `mu_peak=max_t M||P_Mu(t)||_2^2`,

`N W_UV^abs <= [3 sqrt(pi)/sqrt(R(R-1))] sqrt(mu_peak) D_tail`,

provided the lower UV parent cutoff lies above `N`.  Since the selected output shell carries at least `p_max nu D_tail/2`, the theorem core is the **continuous** locality-radius relation

`W_comp,R exp(H_inf^out)/D_tail + [3 sqrt(pi)/sqrt(R(R-1))] sqrt(mu_peak) exp(H_inf^out) >= nu/2`.

This one inequality is canonical; `R` is a physical locality radius parameter, not a stop class.  The balanced dyadic `R=2` split is only a readable corollary:

`mu_peak exp(2 H_inf^out) >= nu^2/(72 pi)`

or

`W_comp,2 exp(H_inf^out) >= nu D_tail/4`,

with exact ties allowed to satisfy both.  The first owner is an actual hard critical-shell event and enters the generic shell first-stop/service theorem.  On the second owner, both parent frequencies are at most `2M`; **only after this Fourier locality statement has been proved** is exact coherent Hahn atomization applied to the restricted comparable source.  Locality is therefore a property of the PDE triad law, not of coherent representation.

Dedicated GitHub Actions run `31350897986` on exact `e7cdb0d6645574898a3900cbccc904949bd7e0a1` passed the full theorem suite, `50,000` ultraviolet locality states, and direct common-unit high-tail, physical pair-productivity and generic critical-shell dependencies.  Stress reported exact `3 sqrt(pi)` constant residual `0`, minimum continuous tradeoff margin `7.675e-06`, minimum dimensionless balanced-owner margin `5.070e-05`, minimum localized binary positive-dominance margin `4.025e-06`, maximum joint clean owner count `2`, and `43,657/50,000` sampled cases in which a fake atomic-positive reweighting changed the output-scale `p_max`, confirming that aggregate HH locality and coherent Hahn positivity are genuinely different observables.  Stored artifact: `recorded-results/31350897986/`.  Full physical-energy causal integration `31350897937` on the same exact SHA completed successfully through the locality theorem, physical productivity, recursive witness construction, Shannon/Renyi, branch compiler and master episode stress.

Status: `EXACT_HIGH_TAIL_HH_OUTPUT_SCALE_LAW__ULTRAVIOLET_WORK_TO_PHYSICAL_DTAIL__CONTINUOUS_LOCALITY_RADIUS_MASS_TRADEOFF__COMPARABLE_REMAINDER_BINARY_READY__NO_PAIR_COUNT_OR_COHERENT_LOCALITY_SELECTION`.
...
### High-tail temporal seam closure: sliding natural-window reentry

The temporal relation above is now theorem-level.  The key correction is to keep the high-tail work law in the already-certified common causal unit `N dW` and to attach every concentration statistic to the **actual selected hard-shell natural window**.  If the selected high-tail shell is

`M=2^j N`, `j>=1`,

then

`T_N=cN^-2`, `T_M=cM^-2=4^-j T_N<=T_N/4`.

For the actual positive comparable-HH work measure `mu_comp` on the parent block define the sliding concentration

`p_t = sup_s mu_comp([s,s+T_M])/mu_comp(I)`,

`H_inf^time=-log p_t`.

No time partition is introduced.  For smooth Navier--Stokes the positive comparable-work density is continuous, so the sliding integral is continuous in the start time and attains its maximum on the compact admissible interval.  Translation of the time origin, simultaneous rescaling of the time unit and density, and subdivision of a representation of the same positive measure all leave `p_t` unchanged.  The theorem API now also requires the supplied window length to equal the selected shell's exact `cM^-2`; a concentration number from another scale cannot be attached by hand.

The canonical strict cutoff currently gives only `|S|<=1`, hence universally

`||h||_2=||(I-S)u||_2<=2||u||_2`.

For both comparable HH parents in `|xi|<=RM`, sharp physical Young plus Fourier-volume Holder gives the physical work-rate bound

`r_comp <= 12 sqrt(pi) R M^2 E_global sqrt(mu_win)`,

where

`mu_win=max_(t in I_*) M||P_Mu(t)||_2^2`, `E_global=sup_t||u(t)||_2^2`.

Integration on exactly one `M`-natural window cancels `M^2`:

`W_win^phys <= 12 c sqrt(pi) R E_global sqrt(mu_win)`.

Returning to the common high-tail causal unit gives

`N W_win <= 12 c sqrt(pi) R N E_global sqrt(mu_win)`.

The Fourier-locality theorem supplies, on the clean comparable owner,

`W_comp/p_scale >= nu D_tail/4`,

while the sliding measure gives

`W_win=p_time W_comp`.

Therefore

`boxed: sqrt(mu_win)/(p_scale p_time) >= nu D_tail/[48 c sqrt(pi) R N E_global]`.

Equivalently, with the scale-critical parent energy `mathcal E_N=N E_global`,

`boxed: sqrt(mu_win) exp(H_inf^out+H_inf^time) >= nu D_tail/[48 c sqrt(pi) R mathcal E_N]`.

For the dyadic locality corollary `R=2`, the denominator is `96 c sqrt(pi) mathcal E_N`.

The resulting `mu_win` is an **actual hard-shell event** of the full velocity, so it enters the material-free generic critical-shell first-stop theorem.  Own-scale bounded service is still registered only conditionally on a full no-hit renewed shell corridor.  Hard-tail support simultaneously gives genuine forward progress

`M/N=2^j>=2`

and natural-time shortening

`T_M/T_N=4^-j<=1/4`,

independently of signed-good Young geometry.

The first candidate SHA `ea9e89efa4608e2bb7a369ad0e0716055ff78bd9` reached `639` passing tests and exactly two fixture failures before any theorem stress ran.  Both fixtures had fixed `E_global=100`, which made the capacity-implied window mass `1.6545e-9` exceed the fixture's certified block peak `1e-10`; the second intended wrong-window test therefore hit the mass guard first.  No theorem equation was changed.  Commit `6c42d53f4f3903d141986d85a5b45954c27c18c8` changed only those fixtures by choosing their energy from the certified block-mass cap.

Dedicated GitHub Actions run `31354438956` on exact `6c42d53` passed `641` tests, `50,000` sliding-measure / scale-time / shell-reentry states, the ultraviolet-locality dependency and the generic critical-shell dependency.  Stress reported:
...
- minimum forward-scale margin above the sharp lower boundary `2` equal to `0`;
- minimum natural-time shortening margin below the sharp upper boundary `1/4` equal to `0`;
- minimum conditional full-survivor own-scale service lower `9.309745536273158e-17`.

Stored artifact: `recorded-results/31354438956/`.

Manual full physical-energy causal integration `31354509984` on the **same exact SHA** completed successfully through the new temporal theorem, outer-role/material/interface machinery, physical productivity, recursive witness construction, Shannon/Renyi reuse, branch compiler and master episode stress.

Status: `EXACT_HIGH_TAIL_COMPARABLE_HH_TO_SLIDING_NATURAL_WINDOW__TIME_ORIGIN_AND_UNIT_INVARIANT__SCALE_TIME_CONCENTRATION_TO_CRITICAL_SHELL__NO_PACKET_PERSISTENCE_OR_TIME_BINNING`.

This closes the high-tail regeneration continuation at the level

`physical cause -> Fourier scale -> natural time -> actual critical hard-shell event`.

It does **not** by itself prove that every recursive master path has supplied signed-good scale geometry, nor does it promote the conditional full-survivor shell service to an unconditional event.  The remaining programme-level task is the final continuum master assembly: compose the now-certified supplier routes without double counting owners, without observer-dependent clocks/partitions, and with only genuinely globally bounded scale-independent resources allowed to terminate recursion.  No 3D Navier--Stokes global-regularity theorem is claimed.
