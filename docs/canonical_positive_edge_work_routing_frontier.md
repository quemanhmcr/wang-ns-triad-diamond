...
Status: **CANONICAL ROUTING THROUGH HARD-TAIL TRUE-UPWARD SUPPLY CERTIFIED**.  Mixed fate is no longer a binary Young obstruction; canonical `dW^-` has exact same-triad donor provenance into the already-fixed `dW^+`, hard coarsening charges recipients once, mode-set continuity carries persistent stock, radial exteriors distinguish true crossing from circulation, and the hard-tail theorem now identifies actual `Phi_up`—not gross high-tail traffic—as UV supply.  Pure-UV upward atoms are forced into the first shell `M=2N` with comparable parents; deeper upward atoms have resolved-scale parent contact.  Remaining local HH seams are degenerate full-signed Young/Christ margin and any genuinely necessary coherent positive kernel; remaining scale work is branch-specific positive binding/continuation, not temporal matching or a gross-transfer budget.
...
Thus overlapping donor cells disintegrate one canonical recipient charge rather than cloning it.  Coarse self-loops remain real same-time redistribution with zero event depth and no scale progress; mixed-fate failure domination is still not a payment map.  Mode-set continuity (`a39d502...`) places stock on physical helical modes; radial crossing (`667e687...`) removes high→high circulation from Fourier-sphere supply; and `hard_tail_true_upward_supply.md` (`d064bc4...`) closes the next scale interface by replacing gross tail regeneration with actual low→high `Phi_up` in the common `N dW` unit.  The new open scale seams are positive binding of resolved-contact atoms and continuation of the already-first-shell comparable pure-UV submeasure.
