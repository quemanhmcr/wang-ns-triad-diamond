...
Accordingly, generic bad HH routing and terminal-bad assistance in mixed cells are no longer open recursive branches.  On low native-deficit blocks, nondegenerate full-signed Christ-margin failures are quantitatively dominated by actual canonical negative work plus already-terminal bad-positive work through an explicit `dW/dA>19/100` good-core comparison; no capacity probability is made causal.  The cyclic theorem closes same-time negative-work provenance, the hard-cell theorem closes single charging, the helical mode-set theorem closes between-time stock/boundary-flow/viscosity registration, and the radial theorem now distinguishes actual low→high/high→low Fourier-sphere crossing from high→high circulation.  Its `dR/R` layer cake is physical log-radius displacement, not a new clock, and equiradial positive transfer proves that transfer alone supplies no minimum scale progress.  The sharp local HH seam remains degenerate full-signed Young/Christ margin and any genuinely required coherent positive kernel; the next scale-facing PDE seam is to interface **true upward boundary supply** with the existing hard-tail dissipation/locality route.  No global-regularity claim is made.
...
The identity uses the already-canonical gross edge `dW+`/`dW-` marginals and viscosity; it does not Hahn-split modal net work, does not pair prior deposits with later withdrawals, and does not bound gross traffic.  The physical amplitude-scaling anti-theorem keeps a closed triad's full-set boundary flux at zero while internal work scales cubically.  The radial specialization is now certified: for `H_R={|k|>R}`, nonlinear tail divergence is exactly `Phi_up(R)-Phi_down(R)`, high→high circulation cancels, and the truncated `dR/R` layer cake equals the corresponding clipped log-radius displacement of the same donor/recipient atoms.  Equiradial transfer can have positive work and zero radial action.  The next native question is whether the hard-tail supplier can be rewritten/strengthened so the UV input is actual `Phi_up`, with inherited stock, `Phi_down`, high→high circulation and viscosity kept physically distinct.
