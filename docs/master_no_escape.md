...
Write `L_V=K+S`, with `K^*=-K` and `S^*=S`.  Across the complete square
partition, partition motion plus `K` has an exact antisymmetric pair matrix whose
row sums are the smooth relink works; its total is zero.  The `S` pair matrix is
symmetric and reconstructs the same resolved strain/deformation work.  This is
the type-correct smooth-carrier theorem.
...
complement restores zero.  A large interface coefficient impulse is only an
interval locator; actual `Q^2` energy and native work reenter the physical-energy
gate before relink, strain, inheritance or HH generation is named.

Dedicated run `31401197668` and full integration `31401197364` certified this
handoff on exact theorem SHA `54bf268fd363a3778627e281756482c64f37ae97`.
The dedicated run passed `671` tests and `50,000` new states; artifacts are under
...
A failed common-slice mark cannot be renamed packet decoherence; it carries
source/relink or earlier HH-regeneration provenance.  The exact initial boundary
remains absorbing.
