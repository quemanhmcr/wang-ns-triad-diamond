...
A strain/interface/HH first stop inside that interval is event-facing after the
required physical-energy routing; `t=0` remains absorbing.  But if no stop occurs
and the whole natural interval survives, the earlier endpoint is present only
because the proof chose one `A`-natural horizon.  The elapsed corridor time is
physical; the horizon endpoint is an **analysis checkpoint** and contributes zero
recursive event vertices.
...
- an event-free UV-growing sequence of no-hit **checkpoints**, whose real physical
...
Accordingly, generic bad HH routing and terminal-bad assistance in mixed cells are no longer open recursive branches.  On low native-deficit blocks, nondegenerate full-signed Christ-margin failures are quantitatively dominated by actual canonical negative work plus already-terminal bad-positive work through `dW/dA>19/100`; no capacity probability is causal.  Cyclic donor provenance, single charging, helical-mode stock continuity, and radial crossing are certified.  The subsequent hard-tail true-upward theorem now closes the scale-facing interface: actual low→high `Phi_up`, not gross high-tail traffic, is the UV feeder; pure-UV supply is first-shell/comparable, while deep supply has resolved-scale parent contact.  The sharp local HH seam remains degenerate full-signed Young/Christ margin and any genuinely required coherent positive kernel.  The next scale questions are branch-specific positive binding/continuation.  No global-regularity claim is made.
...
The identity uses the already-canonical gross edge `dW+`/`dW-` marginals and viscosity; it does not Hahn-split modal net work, pair prior deposits with later withdrawals, or bound gross traffic.  Radial specialization gives `Phi_up-Phi_down` and a physical `dR/R` layer cake while equiradial transfer shows that work need not imply scale progress.  The hard-tail true-upward theorem now completes the next native step: inherited stock and actual `Phi_up` are the only positive suppliers in the tail owner cover; high→high circulation stays internal, `Phi_down` stays a drain, and viscosity stays loss.  Upward supply then splits atomwise into first-shell comparable pure-UV HH or resolved-scale parent contact.


## Current hard-tail true-upward refinement

The certified scale-facing refinement is now

`physical D_tail -> inherited tail stock OR actual low→high Phi_up`.

On the `Phi_up` branch, the physical donor is one recipient interaction parent.  If both interaction parents are above `M/4`, donor `<=N` forces `M=2N` and triad closure gives comparability.  Any direct upward atom into `M>=4N` instead has a parent at `<=M/4` and is only classified as resolved-scale contact until a separate positive smooth-interface binding is proved.  The two submeasures may coexist in one closed-triad law.  This refinement uses the parent-block common unit `N dW`, introduces no re-Hahn operation, and supplies no additive reset or global regularity conclusion.
