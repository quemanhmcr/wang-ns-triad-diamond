...
The unresolved master step is now ancestor-reservoir synchronization: a winning
coarse/base packet generated by the Onsager collision must be matched across
successive high-frequency blocks and shown to pay fresh radius, reuse holonomy,
Bellman/cycle entropy, or dissipation.  Pressure-third near-field registration
and selected-interface `Xi` summability remain separate continuum tasks.
