...
That interface is now certified in `hard_tail_true_upward_supply.md` on exact theorem SHA `d064bc4d780d9c90d36e64c7b84c3b771b74c896`: hard-tail dissipation is continued by inherited stock or actual `Phi_up`, never by high→high circulation, while the common parent-scale unit `N dW` is preserved.  The resulting upward law splits atomwise into pure-UV first-shell comparable supply and deep resolved-scale parent contact.
...
This theorem does not prove finiteness of total upward crossing over a singular-time history, positive average radial drift, or recurrence termination.  Its hard-tail energy interface is now supplied by `hard_tail_true_upward_supply.md`; the remaining branch-specific questions are positive binding of resolved-scale contact and continuation of the already-comparable pure-UV first-shell submeasure.  No Navier--Stokes global-regularity claim is made.
