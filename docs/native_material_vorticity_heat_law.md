...
For the material NS metric, choose a `G`-orthonormal frame and extend the trace-free
three-dimensional metric velocity by a zero harmonic-depth direction.  Its spectrum is
`(s_1,s_2,s_3,0)` with `s_1+s_2+s_3=0`.  The stress spectrum above and von Neumann's
trace inequality give the sharp algebraic gap

\[
\boxed{|T:\dot G|\le\sqrt{\frac23}\,|T|\,|\dot G|.}
\]

Equality forces `(s_1,s_2,s_3)` proportional to `(2,-1,-1)` up to sign/permutation.
This is the same Weyl-wall equality geometry as

\[
\boxed{|\det K|\le\frac{|K|^3}{3\sqrt6}.}
\]

Thus Maxwell work cannot be perfectly aligned with an incompressible material metric
velocity.  The shared equality wall is a primitive self-frustration, but `det K` is
signed, so it is not a monotonicity theorem.
...
claimed.  This material law therefore constrains the historical no-Zeno heat-frame
problem rather than supplying a pointwise source-versus-heat inequality.
