...
A concrete routing-stress lesson motivated this rule.  The reference harness took `62.50 s` for `1,000` samples on one cloud CPU while using only about `40 MB` RSS.  Profiling showed that most time was not spent on the routing inequalities: every sample rebuilt the same three helical geometries/eight-sector registrations and repeated verified-ledger work even though only the quotient-measure weights and `tau` changed.  Registering that immutable physics once, rescaling only the mathematically varying measure masses, caching immutable atom data, and removing one redundant replay reduced the same `1,000`-sample run to `2.55 s` (about `95.9%` less wall time); the optimized `50,000`-sample run completed in `117.83 s` on one core with about `39.5 MB` peak RSS.  A `64`-case old-path/new-path differential check matched the full dataclass results bit-for-bit.  The lesson is not "cache aggressively": it is **identify the theorem invariants first, then stop recomputing them, and prove the optimization equivalent to the readable reference path**.
