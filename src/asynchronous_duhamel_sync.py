...
SYNC_CONE = Fraction(3, 8)
SYNC_FIXED_POINT = Fraction(10, 39)
COMMON_SLICE_MARGIN = Fraction(9, 40)
...
def synchronization_cone_margin(alpha: float = float(SYNC_CONE)) -> float:
...
def common_reference_slice(interval_start: float, interval_end: float, min_lifetime: float) -> float:
    ...
    if alpha > float(SYNC_CONE) + 1e-14:
        raise ValueError("event slab lies outside the certified synchronization cone")
...
def minimum_backward_displacement(initial_lifetime: float, layers: int) -> float:
    """Minimum cumulative physical time moved backwards by L common slices.

    Each earlier causal generation has lifetime at least (64/25) times the
    preceding one, while every synchronization step moves backwards by 2/5 of
    that layer's minimum lifetime.
    """
    ...
    b = float(BACKWARD_FRACTION)
    return b * initial_lifetime * (g**layers - 1.0) / (g - 1.0)
...
def interior_depth_upper(current_time: float, initial_lifetime: float) -> int:
    ...
    g = float(LIFETIME_GROWTH_MIN)
    b = float(BACKWARD_FRACTION)
    x = 1.0 + (g - 1.0) * current_time / (b * initial_lifetime)
...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "invariant_cone": "alpha <= 3/8",
        "common_slice_margin": "1-3/8-2/5 = 9/40",
        "fixed_point": "10/39",
        "backward_displacement": "Delta t_L >= (10/39)T_0[(64/25)^L-1]",
