...
def normalized_covariance_eigenvalue_bounds(
    ...
    """Eigenvalue bounds for normalized physical covariance N^2 Sigma."""
    if radius_cap <= 0 or aspect_cap < 1 or lower_axis_constant <= 0:
        raise ValueError("invalid radius/aspect shell data")
    eig_lo = lower_axis_constant**2
    eig_hi = aspect_cap ** (4.0 / 3.0) * radius_cap**2
...
def effective_root_frame_budget(
    ...
    return float(bins * phase_space_color_count(4, 6)) * float(CLEAN_BESSEL)
...
def theorem_certificate() -> dict[str, object]:
    ...
    return {
        ...
        "covariance_cover": f"transition-aspect, 2/3<Nr_g<=4 covariances need at most {bins} log-SPD bins of radius 0.4",
