...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language now has the lower covariance-stress form: U+V=E, Euler exchanges U<->V through tau_y, and Z-Z_y splits into productive plus two frustration remainders; the remaining unproved theorem is a no-boundary-concentration history bound preventing scale-invariant A_y depth area from repeatedly narrowing while A'_0 regenerates",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel problem, infinite productive regeneration language and quantitative stability/history gap now sit above one covariance boundary-stress statement: |R_E|^2<=4 A_y(Z-Z_y) and heat pays Z-Z_y, but the transverse activity A_y has no proved finite-history budget; controlling its physical regeneration from the tau cocycle/Loewner/divergence laws is the unproved step",
