...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_weighted_jacobi_continuation": "because c_IJK=<e_I,[e_J,e_K]>=lambda_I f_IJK, ordinary Lie Jacobi gives A+B+C=0 with A=sum_M lambda_M f_MJK f_LIM and cyclic B,C; hence |A|^2<=2(|B|^2+|C|^2) and max(|B|,|C|)>=|A|/2, unchanged after multiplying by z_I z_J z_K z_L",
        "primitive_jacobi_biot_savart_seam": "Jacobi forces companion two-interaction coefficients but not a heat bill: the intermediate lambda_M in c_MJK=lambda_M f_MJK may cancel the 1/lambda_M from C^-1 when that vorticity carrier is reused as velocity; Galilean null and nu|lambda_M|^2 heat are available but the state-weighted companion-to-heat bridge is unproved",
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language now bottoms out at fixed two-road dynamics u_t=J_u r-nu C^2u: one-step productive orientation has no sign monotone, while repeated continuation is constrained by weighted Jacobi companions; the unproved theorem is the state-weighted bridge from compulsory two-step companion activity to heat-visible carrier cost after the lambda_M versus C^-1 cancellation",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel, infinite productive regeneration and quantitative stability/history readings are now a continuation problem: kappa may be productive and keep increasing for one step, but any two-step continuation coefficient belongs to a weighted Jacobi triple with a comparable companion; what remains unproved is converting that companion, despite Biot-Savart cancellation, into finite-time heat-paid persistence control strong enough to make normalized escape action integrable",
