...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "uv_curvature": "for every strict heterochiral UV spread, M^2 F_log<=Q_triangle=M^2(1-D)(1+S)(D+S)R; constant 1 is sharp along D=1-epsilon^2,S=epsilon where retained child/donor work tends 1 while F_log tends 0, so near-perfect retention is permitted only with vanishing logarithmic scale displacement",
        "primitive_fixed_loss_window_guard": "the signed-good 10/13 child/donor ceiling is a local 3/5--5/8 window theorem, not a global primitive law: on the full strict physical UV triangle (D+S)/(1+S) can approach 1",
        "primitive_positive_curvature_composition_guard": "Q_triangle is positive for one oriented triad occurrence, but summing Q_triangle^+ creates a representation-dependent gross-traffic budget; full Nijenhuis curvature is representation-free but signed, so a positive many-triad composition theorem remains unproved",
        ...
        "lax_material_heat": "with Delta_op=sum_j[D_j,[D_j,.]]>=0 and [D_j,ad_omega]=ad_(D_j omega), full NS obeys (ad_omega)_t=[ad_omega,ad_u]-nu Delta_op ad_omega; after material pullback the vorticity 2-form obeys the equivalent geometric heat law in the moving metric",
        "primitive_normalized_projector_two_road": "for e=u/sqrt(E), P=e tensor e and A_e=sqrt(E)(J_e C+C J_e), one has A_e*= -A_e, A_e e=F_E/sqrt(E), and exactly P_t=[A_e,P]-nu[P,[P,Lambda^2]]; this is not autonomous in P because e->-e fixes P and reverses A_e; the viscous road satisfies d<e,Lambda^2 e>/dt|_nu=-2nu||(Lambda^2-N^2)e||^2<=0",
        "primitive_corotating_heat_frame": "with mathscrA(u)=J_u C+C J_u, mathscrA*= -mathscrA and mathscrA u=F_E; if U_E,t=mathscrA U_E then U_E is unitary and tilde u=U_E* u, tilde Lambda=U_E* Lambda U_E obey exactly tilde u_t=-nu tilde Lambda^2 tilde u and tilde Lambda_t=[tilde Lambda,tilde mathscrA]; NS is heat of the state in a self-rotating isospectral curl frame, not a fixed autonomous heat semigroup",
        "primitive_frame_commutator_current": "with B=[Lambda,mathscrA]=B*, m=<e,Lambda e>, delta^2=||(Lambda-m)e||^2 and n=(Lambda-m)e/delta, Euler gives m'_E=<e,B e>, delta'_E=<n,B e> and n'_E-mathscrA n=delta^-1 Pperp B e; hence B e=m'_E e+delta'_E n+delta(n'_E-mathscrA n), so scale growth, spread and turning are projections of one frame current",
        "primitive_krein_lax_boost": "on the curl-invertible range with (x,y)_G=<Lambda^-1 x,y>, J=sgn C and U=-ad_u, helicity ad-invariance gives U_G^dagger=-J U J and Sym_G U=(U-J U J)/2; since ||omega||_G^2=K, exactly K'_E=2(omega,Sym_G U omega)_G=2kappa",
        "primitive_radial_tilt_commutation": "on the normalized radial probability simplex V_g(p)=2(g-E_p g)p gives [V_r,V_(r^2)]=0; this rules out a hidden noncommutative self-turning mechanism between the canonical radial productive and heat tilt generators, but not complete physical-time commutation because a/delta is endogenous",
        "primitive_orientation_covariant_law": "with A=a-nu delta(m+alpha1), W=w-nu delta h, v=e_t=A n+W and Pperp=I-e tensor e-n tensor n, exactly Pperp n_t=delta^-1[A h+Pperp(Lambda-m)W] and D_t w=sqrt(E)Pperp(J_v C e+J_e C v)-nu N^2 w-(a/delta)[A h+Pperp(Lambda-m)W]",
        ...
        "primitive_euler_kappa_derivative": "with F=J_u C u and kappa=<Lambda u,F>, the pure-Euler derivative is kappa'_E=||Lambda^(1/2)F||^2+<Lambda u,J_F C u>+<Lambda u,J_u C F>; only the first term has fixed sign",
        "primitive_euler_acceleration_leaf": "Euler invariance fixes the normal acceleration exactly: <u,F'>=-||F||^2 and <Cu,F'>=-<CF,F>; if P_span{u,Cu} Lambda u=a u+b Cu and g_K=P_T Lambda u, then kappa'_E=<F,(Lambda-a-bC)F>+<g_K,P_T F'>, so the remaining freedom is tangent turning",
        "primitive_absolute_curl_productive_frame": "with e=u/sqrt(E), m=K/E, delta^2=(EZ-K^2)/E^2, n=(Lambda-m)e/delta and f=F_E/sqrt(E)=a n+w, w perpendicular e,n, one has a=kappa/sqrt(EZ-K^2), A_escape=a^2/N^2, m'_E=2a delta and delta'_E=a(alpha1-m)+<h,w> for h=P_{e,n}^perp Lambda n",
        "primitive_absolute_curl_self_turning": "the exact moving-frame law is n'_E=-a e+delta^-1[a h+P_{e,n}^perp(Lambda-m)w]; hence for w=0 the intrinsic tangent turning is (a/delta)h, so productivity couples to self-turning only through the endogenous next absolute-curl Krylov opening and no uniform turning gap is claimed",
        "primitive_helicity_side_motion": "Euler helicity tangency gives a c_H+<v_H,w>=0 with c_H=<Ce,n> and v_H=P_{e,n}^perp Ce; if v_H is nonzero then ||w||^2>=a^2|c_H|^2/||v_H||^2, while v_H=0 means the exact constraint is a c_H=0 and no 0/0 quotient is used",
        "primitive_toda_saturation_flow": "if w=0, the radial Lambda spectral law obeys p_t(r)=2(a/delta)(r-m)p(r); after the endogenous time change ds/dt=a/delta this is the first Toda spectral-measure flow, with m_t=2a delta and delta_t=a(alpha1-m); this is a saturation-branch identity, not a global integrable reduction of NS",
        "primitive_two_radius_krylov_rigidity": "for delta>0, h=0 iff span{e,n} is Lambda-invariant, so the cyclic absolute-curl spectral measure is supported on at most two radii; exact no-turning saturation is therefore two-radius radial geometry rather than a mechanism for creating new radii",
        "primitive_turning_reconfiguration_balance": "with Theta=P_{e,n}^perp(n'_E+a e) and R_w=P_{e,n}^perp(Lambda-m)w, exactly delta Theta=a h+R_w and a^2||h||^2<=2 delta^2||Theta||^2+2||R_w||^2; this exposes turning/reconfiguration but does not by itself control a^2 because h may vanish and R_w carries an extra |C| weight",
        "primitive_one_step_turning_guard": "one fixed physical Fourier triangle/helicity assignment realizes all four sign quadrants of (kappa,kappa'_E) by changing only complex amplitudes, and normalized signed productivity can be positive and increasing; there is no one-step sign-decay theorem, even though the absolute-curl productive frame has an exact h-weighted self-turning identity",
        "primitive_full_two_step_gap": "one actual nested Euler path (J,K)->M then (I,M)->L carries coefficient (lambda_K-lambda_J)(lambda_M-lambda_I) f_MJK f_LIM; the intermediate signed curl survives as a gap, so equal-curl/degenerate continuations can vanish exactly",
        "primitive_weighted_jacobi_continuation": "because c_IJK=<e_I,[e_J,e_K]>=lambda_I f_IJK, ordinary Lie Jacobi gives bracket-level A+B+C=0 with A=sum_M lambda_M f_MJK f_LIM and cyclic B,C; hence |A|^2<=2(|B|^2+|C|^2) and max(|B|,|C|)>=|A|/2, unchanged after multiplying by z_I z_J z_K z_L",
        "primitive_full_continuation_companion_guard": "weighted Jacobi does not imply comparable full Euler companions after signed-curl gaps are restored: a physical helical four-mode witness has one nonzero full nested continuation while both cyclic full companions vanish, even though the bracket-level weighted Jacobi identity remains exact",
        "primitive_fourier_diamond_carrier_guard": "for m1=j+k, m2=k+i, m3=i+j and ell=i+j+k, exactly m1+m2+m3=2ell and max |m_r|>=2|ell|/3; this forces a comparable-scale intermediate location but not nonzero companion work on that carrier",
        "primitive_jacobi_biot_savart_seam": "Jacobi controls bracket compositions rather than a heat bill: actual Euler reuse also carries signed-curl gaps, while converting intermediate vorticity to velocity introduces C^-1; Galilean null and nu|m|^2 heat are exact endpoint constraints, but the representation-free positive composition bridge is unproved",
        ...
        "primitive_poisson_product_defect": "with Gamma(f,g)=f Lambda g+g Lambda f-Lambda(fg), P_y(fg)-P_yf P_yg=int_0^y P_(y-s)Gamma(P_sf,P_sg)ds; D_y=P_y(u cross omega)-(P_yu) cross(P_yomega) is both the Gamma_cross integral and -div(tau_y-(tr tau_y)I/2), while R_E(y)=2<P_yu,P D_y>",
        ...
        "primitive_poisson_depth_two_road": "mathfrak q=2||Lambda P_yu||^2=Escript_yy/2 obeys q_t+partial_y(j_E+(nu/2)q_y)=0 with j_E=2<Lambda P_yu,P_yF_E>=-R_E,y/2, int j_E dy=0, q_y<=0, j_E(0)=2kappa and (nu/2)q_y(0)=-2nu M3; Euler is a zero-depth-integral return current while heat has one sign",
        "primitive_poisson_hankel_escape": "for q=2||Lambda P_yu||^2, M0=int q=K, M1=int yq=E/2, q(0)=2Z and M1 q(0)-M0^2=EZ-K^2; the exact escape density is j_E(0)^2 M1/[q(0)(M1 q(0)-M0^2)], while zero defect forces the equiradial state and j_E(0)=2kappa=0",
        ...
        "primitive_poisson_covariance_stress": "with v=P_yu and tau_y=P_y(u tensor u)-v tensor v, positivity/Markov gives tau_y>=0 and tau_0=0; exactly (partial_y+Lambda)tau=Gamma_v and v_t=P(v cross Cv)-P div tau-nu Lambda^2 v, so the scalar Poisson profile is a shadow of a lossless positive covariance-stress lift",
        "primitive_harmonic_halfspace_normal_form": "with Delta4=partial_y^2+Delta_x, the canonical lift obeys Delta4 v=0 and, for pi=P_y p, v_t+(v.grad_x)v+div_x tau+grad_x pi=-nu v_yy with -Delta_x pi=partial_i partial_j(v_i v_j+tau_ij); this is a local pressure form of the same lossless Poisson lift",
        "primitive_harmonic_covariance_source": "because P_y(u tensor u) and v are harmonic, -Delta4 tau=2 sum_A partial_A v tensor partial_A v>=0; for C=(tr tau)I-tau, -Delta4 C=2 sum_A(|partial_A v|^2 I-partial_A v tensor partial_A v)>=0 and -Delta4(a^T C a)=2 sum_A|a cross partial_A v|^2 for constant a",
        "primitive_harmonic_critical_energy": "the canonical half-space field satisfies int_R4+ |grad_4 v|^2=K and int_R4+ |grad_4^2 v|^2=2M3; also V_y(0)=2K, V_yy(0)=-4Z and V_yyy(0)=8M3, so the critical hierarchy is boundary geometry of one Dirichlet-generated covariance potential",
        "primitive_harmonic_concentration_guard": "under NS dilation v_lambda(x,y,t)=lambda v(lambda x,lambda y,lambda^2 t), so int_R4+|grad_4 v_lambda|^2=K and A_lambda(y,t)=lambda A(lambda y,lambda^2 t); fixed-time Dirichlet/depth-area bounds therefore do not by themselves penalize canonical boundary concentration, and a dynamical no-concentration theorem remains unproved",
        "primitive_poisson_germano_cocycle": "the covariance is depth-consistent without a shell reset: tau_(y+z)(u)=P_z tau_y(u)+tau_z(P_yu), both terms are PSD, and after trace integration V_u(y+z)=V_u(y)+V_(P_yu)(z)",
        "primitive_poisson_two_reservoir_exchange": "U_y=||P_yu||^2 and V_y=int tr tau_y obey U_y+V_y=E with V_0=0 and (1/2) partial_y V|_0=K; Euler gives U_t|E=2 int tau_y:S(P_yu)=-V_t|E, while viscosity gives U_t|nu=-2nu Z_y and V_t|nu=-2nu(Z-Z_y)<=0, so there are two reversible-exchange reservoirs and heat removes from both",
        "primitive_poisson_stress_divergence": "D_y=P_y(u cross omega)-(P_yu) cross C(P_yu)=1/2 grad tr tau_y-div tau_y=-div(tau_y-(tr tau_y)I/2); hence the Euler product defect is the divergence of the stress-energy tensor of the positive velocity covariance",
        "primitive_poisson_covariance_heat_fisher": "pure heat obeys (partial_t+nu Lambda^2)tau_y=-2nu sum_k tau_y(partial_k u)<=0 and sum_k int tr tau_y(partial_k u)=Z-Z_y; covariance Cauchy gives int |D_y|^2/tr tau_y <= Z-Z_y with constant 1, using quotient 0 on the zero-trace set",
        "primitive_poisson_covariance_boundary": "as y->0, tau_y/y->Gamma_u and D_y/y->Gamma_cross, so Gamma_cross=1/2 grad tr Gamma_u-div Gamma_u, int |Gamma_cross|^2/tr Gamma_u<=2M3 and <u,Gamma_cross>=int Gamma_u:S=-2kappa",
        "primitive_poisson_transverse_activity": "with C_y=(tr tau_y)I-tau_y and A_y=int v^T C_y v, D_y lies in Ran C_y and a_y=C_y^dagger D_y is the canonical least-squares axis; A'_0=2||A_u||_pair^2 on continuum R3, while Germano gives A_(y+z)=A_z(P_yu)+a second nonnegative inherited-covariance term but does not make A_y monotone",
        "primitive_poisson_covariance_pythagoras": "exactly Z-Z_y=R_E^2/(4A_y)+R_align+R_reg for A_y>0, with both remainders nonnegative; if A_y=0 then R_E=0 and the unsquared regression identity is used",
        "primitive_poisson_relative_completed_square": "V_t=A_y/(2nu)-nu(R_E+A_y/nu)^2/(2A_y)-2nu R_align-2nu R_reg for A_y>0; its y->0 germ is K'=(||A_u||_pair^2-||A_u-2nu Domega||_pair^2)/(2nu)",
        "primitive_poisson_axis_gradient_guard": "where a_y=C_y^dagger D_y is sufficiently regular, int a_y^T C_y a_y=int (tau_y-(tr tau_y)I/2):grad a_y; rank changes of the pseudoinverse prevent treating this as an unconditional global coercive grad-a estimate",
        "primitive_poisson_activity_depth_area_guard": "pointwise v^T C_y v=e2(P_y(u tensor u))-e2(tau_y)<=(P_y|u|^2)^2/3, so on continuum R3 int_0^infinity A_y dy<=(1/6)|||u|^2||_Hdot^-1/2^2 lesssim K^2; A_(u_lambda)(y,t)=lambda A_u(lambda y,lambda^2 t), so finite depth area does not prevent a narrowing boundary layer with slope scaling lambda^2",
        "primitive_poisson_dirichlet_local_current": "rho=|grad_4 v|^2 obeys rho_t+div_4 J=0 with J_A=-2 v_t dot partial_A v because Delta_4 v=0; after integrating x, J_y becomes j_E+(nu/2)partial_y mathfrak q, so the scalar critical density has no bulk creation term",
        "primitive_poisson_dirichlet_moments": "M0=int mathfrak q dy=K, M1=int y mathfrak q dy=E/2 and mathfrak q(0)=2Z on finite-energy R3 or mean-zero periodic states; hence mean depth=E/(2K), M0'=2kappa-2nu M3 and M1'=-nu Z with zero Euler contribution to M1",
        "primitive_poisson_dirichlet_sobolev_ladder": "mathfrak q is completely monotone with (-1)^n partial_y^n q(0)=2^(n+1)||Lambda^((n+2)/2)u||^2 and int y^n q dy=(n!/2^n)||Lambda^((1-n)/2)u||^2 whenever the displayed jet/moment is finite; on R3 finite energy alone does not force all negative-Sobolev moments",
        "primitive_poisson_dirichlet_tail_law": "K_y=int_y^infinity mathfrak q ds=<P_yu,Lambda P_yu> is exactly the critical stock of the Poisson-smoothed state and (K_y)_t=j_E(y)+(nu/2)mathfrak q_y(y), so the same critical NS law is nested at every canonical depth",
        "primitive_poisson_helicity_depth_split": "q alone forgets curl sign; with the existing J=sgn C, mathfrak q_H=2<Lambda P_yu,J Lambda P_yu> integrates to H and q_pm=(q+-q_H)/2=2||Lambda P_y P_pm u||^2>=0; j_H^E=2<C P_yu,P_yF_E> has j_H^E(0)=0, hence j_+^E(0)=j_-^E(0)=kappa",
        "primitive_poisson_transport_action_guard": "with quotient zero where mathfrak q=0, int j_E(y)^2/mathfrak q(y) dy <= ||Lambda^-1/2 F_E||^2; under continuum NS dilation mathfrak q_lambda(y,t)=lambda mathfrak q(lambda y,lambda^2 t) and j_E,lambda=lambda^2 j_E(lambda y,lambda^2 t), so the spacetime transport action is scale invariant and has no proved finite budget",
        "primitive_poisson_orientation_incompleteness": "fixed physical closed-triad wavevectors/helicities/modal energies determine the same complete radial Dirichlet profile mathfrak q, but a pi common-phase reversal sends every non-affine Euler current j_E(y) and kappa to its negative; no functional of q alone determines nonlinear direction",
        "primitive_poisson_transport_lower_guard": "the lower shortcut A_escape<=int j_E^2/mathfrak q dy is false even on exact physical Waleffe triads; a conditioned deterministic witness has relative Hankel defect 1.464253e-4 and transport/action ratio 3.393252e-6, so scalar depth transport is not a compulsory payment comparable to normalized boundary productivity",
        ...
        "primitive_single_angle_guard": "where cos(theta)=K/sqrt(EZ), Euler obeys -tan(theta) theta'_E=2kappa/K-P/Z with P=int omega.Somega; kappa and P are independent contractions and actual Galerkin sign quadrants rule out a universal one-angle monotone or telescope",
        ...
        "endogenous_euler_coefficient": "with Ec=|C|^-1 Sigma_c |C|^-1, Rc=nu^-1 Ec and z=|C|^(3/2)u, nu_E=kappa/M3=<z,Ec z>/||z||^2 while nu_E/nu=<z,Rc z>/||z||^2; exactly K'=2M3(nu_E-nu), so nu_E is the signed state-generated two-way Euler coefficient and nu is the one-way molecular coefficient",
        ...
        "primitive_productive_pair_projection": "for M3>0, nu_E=kappa/M3 is exactly argmin_lambda ||A_u-lambda Domega||_pair^2; A_u=nu_E Domega+C_perp with C_perp orthogonal Domega, and ||C_perp||^2 cancels identically from ||A_u||^2-||A_u-2nu Domega||^2=4nuM3(nu_E-nu)",
        "primitive_productive_alignment_rigidity": "C_perp=0 makes A_u a two-point coboundary, so A_xy+A_yz+A_zx=(u(x)-u(z)) cross (u(y)-u(z))=0 and the velocity image lies on an affine line; incompressibility gives Galilean shear/null geometry, while finite-energy R3 excludes a nonzero representative; this is equality rigidity, not a uniform gap",
        "primitive_hom_current_lambda_family": "for lambda>0, D_R=lambda d and D_L=lambda d+alpha wedge induce J_lambda=V+lambda B=nabla_lambda A with nabla_lambda J_lambda=lambda(beta wedge)A; hence J_lambda=0 forces the same dimensional-collapse Gauss source, and the physical residual is J_(2nu)=Gc",
        ...
        "primitive_critical_increment_stress_work": "the exact projected divergence is P div Gamma_u=Lambda F_E-P(u cross Lambda omega)-P((Lambda u) cross omega); pairing with u gives <u,div Gamma_u>=2kappa and kappa=-(1/2) int Gamma_u:S, so critical Euler growth is signed work of the positive Lambda increment stress",
        "primitive_critical_increment_transport_guard": "Gamma_u is not a local conformation tensor: for q=u(x)-u(y), its Euler material derivative retains pressure-gradient differences and (q.grad_y)u(y) under the |x-y|^-4 pair kernel, so D_x Gamma+Gamma grad u+(grad u)^T Gamma=0 is false in general and det Gamma is not a material invariant",
        ...
        "primitive_critical_stretching_bridge_guard": "omega(x).(u(x+h)-u(x))=(S omega)(x).h+O(|h|^2), but no static bridge is used: the proposed factor 1/2 is numerically false while a refined physical-Fourier search approaches factor 1 from below without a proof",
        "primitive_two_particle_transport": "with x=c+r/2, y=c-r/2 and q=|delta u|^2/2, exact NS gives q_t+div_c(q ubar+delta p delta u)+div_r(q delta u)=nu(Delta_c/2+2Delta_r)q-nu(|grad u(x)|^2+|grad u(y)|^2); pressure has only the common-coordinate road",
        "primitive_common_relative_pair_law": "U=(u+ + u-)/2 and v=u+-u- obey U_t+U.grad_c U+(1/4)v.grad_c v+grad_c pbar=nu Delta_c U and v_t+U.grad_c v+v.grad_c U+grad_c dp=nu Delta_c v; <U,v>_c=0 and ||U||^2+||v||^2/4=E, with Euler only exchanging their energies",
        "primitive_pair_endpoint_compatibility": "endpoint origin gives grad_r v=grad_c U, 4 grad_r U=grad_c v and Delta_c v=4 Delta_r v, so center relay and separation migration are two coordinate readings of one heat operator",
        "primitive_pair_critical_field": "W=v/|r|^2 has K=(2pi^2)^-1||W||_2^2 and M3=(2pi^2)^-1||grad_c W||_2^2; D_pair W=2 lambda W-|r|^-2 grad_c dp+nu Delta_c W, so critical heat is exactly the center-Dirichlet cost of inverted-pair velocity",
        "primitive_pair_radial_transverse_split": "with n=r/|r| and delta u=a n+b, b perpendicular n, continuum incompressibility gives exactly K_parallel=K/4, K_perp=3K/4 and div_r(delta u)=0; the inverted-pair kinetic metric has the same radial/transverse split",
        "primitive_pressure_pair_converter": "with A3=intint a^3/|r|^5 and B3=intint a|b|^2/|r|^5, kappa=-pi^-2(A3+B3) and exact material pair dynamics gives P_parallel=3(A3-B3)/(2pi^2), P_perp=-P_parallel; pressure converts radial<->transverse critical motion but mints no total critical energy",
        "primitive_relative_pressure_free_law": "Q(r)=int_c q and J(r)=int_c q delta u obey Q_t+div_r J=2nu Delta_r Q-2nu Z; K=pi^-2 int Q/|r|^4 and kappa(0)=-2pi^-2 int J.r/|r|^6, so positive critical Euler work is weighted inward relative transport toward r=0",
        "primitive_inverted_pair_kinetic": "for material r_ab=Phi(a)-Phi(b) and I(r)=r/|r|^2, K=(2pi^2)^-1 int int |d_t I(r_ab)|^2 da db; DI is a scaled reflection, its Hessian cubic gives the kappa integrand, and pressure has zero global work in this inverted-pair kinetic metric",
        "primitive_inverted_pair_history": "K^2<=EZ and -E'=2nu Z imply ||I(r_ab(T))-I(r_ab(0))||_L2 <= sqrt(2pi^2) E0^(1/4) T^(3/4) ((E0-E(T))/(2nu))^(1/4); this is finite Hilbert path length, not endpoint-velocity control or regularity",
        "primitive_pair_affine_defect": "A=(grad u(x)+grad u(y))/2 and C=grad_c v=grad u(x)-grad u(y) obey D_pair C+AC+CA+2 R_c tr(AC)=nu Delta_c C with R_c=grad_c^2(-Delta_c)^-1; the equation is homogeneous in C, global C=0 is affine, and M3 is exactly the critical pair norm of C",
        "primitive_pair_collision_concentration": "fixed-r endpoint energy gives ||v(.,r)||_2^2<=4E and hence K_{|r|>=R}<=8E/(pi R); if K diverges, normalized critical mass converges to the physical collision diagonal r=0 without a shell selector",
        ...
        "primitive_pair_scale_neutrality": "under NS dilation u_lambda=lambda u(lambda x,lambda^2 t), E->lambda^-1 E, Z->lambda Z, K->K, M3->lambda^2 M3, kappa->lambda^2 kappa and ||A_u||_pair^2->lambda^2||A_u||_pair^2; hence both M3 dt and pair-area dt are scale invariant, while shrinking a fixed critical profile does not increase K and escape still requires regeneration/accumulation of critical mass",
        ...
        "primitive_regeneration_persistence": "the historical nu_E=kappa/M3 reading remains exact, and escape necessity int kappa^2/[N^2(EZ-K^2)] dt=infinity is exactly int a^2/N^2 dt=infinity; in the Euler co-rotating frame the radial heat contribution to K/E is nonpositive, so the missing theorem is a no-Zeno historical return law preventing infinite normalized productive frame action in finite smooth time",
        "primitive_affine_core_budget_falsifier": "a hypothetical non-solution core with tau=T-t, |grad u|~tau^-1 and R~tau^alpha has E~tau^(-2+5alpha), K~tau^(-2+4alpha), Z~tau^(-2+3alpha), M3~tau^(-2+2alpha), kappa~tau^(-3+4alpha), and ||A_u||_pair^2~tau^(-4+6alpha); for 2/5<=alpha<1/2 energy and int Z stay finite while K, int M3 dt and int ||A_u||_pair^2 dt diverge, so the energy law does not itself pay the higher Fisher/transverse-covariance bill",
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language now bottoms out at tilde u_t=-nu tilde Lambda^2 tilde u and tilde Lambda_t=[tilde Lambda,tilde mathscrA]: the state has only one-way heat while Euler is two-way isospectral motion of the heat frame; projector, Lax-Krein, Krylov, Poisson and material laws are readings, and the unproved theorem is a no-Zeno frame-history law making int a^2/N^2 dt finite",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel, infinite productive regeneration and quantitative stability/history readings reduce to self-generated heat-frame motion: radial generators V_r and V_(r^2) commute, B=[Lambda,mathscrA] carries scale/spread/turning, and the covariant w plus Lax-Krein laws resolve its orientation; the missing theorem is an exact no-Zeno heat-paid return law, not a final radial norm closure",
        ...
        "graded_current_persistence_guard": "the current algebra does not prove large-data regularity: static Schrodinger coercivity enters the exact/pressure sector and helicity-odd projection loses Gram order; these are higher constraints on the no-Zeno self-rotating heat-frame problem rather than an independent closure theorem",
...
def endogenous_euler_pair_projection(
    pair_area_squared: float,
    third_absolute_curl_moment: float,
    curvature_height: float,
    viscosity: float,
) -> dict[str, float]:
    """Exact pair projection behind the endogenous signed Euler coefficient.

    With ``A_u=u(x) cross u(y)`` and ``Domega=omega(x)-omega(y)`` in the
    canonical critical pair metric, put

        M3=||Domega||^2,  kappa=<A_u,Domega>,
        nu_E=kappa/M3,  A_u=nu_E Domega+C_perp.

    Then ``C_perp`` is orthogonal to ``Domega`` and cancels identically from
    the physical critical square, leaving ``K'=2 M3 (nu_E-nu)``.  This
    helper audits only the exact Hilbert projection algebra; it does not assert
    a quantitative gap away from the equality geometry.
    """
    a2=float(pair_area_squared); m3=float(third_absolute_curl_moment)
    kap=float(curvature_height); nu=float(viscosity)
    if not all(math.isfinite(x) for x in (a2,m3,kap,nu)):
        raise ValueError("finite pair projection data required")
    if a2 < 0.0 or m3 <= 0.0 or nu <= 0.0:
        raise ValueError("nonnegative pair area and positive M3, viscosity required")
    tol=8.0e-12*max(1.0,a2*m3,kap*kap)
    if kap*kap > a2*m3 + tol:
        raise ValueError("pair data violate Cauchy projection constraint")
    nu_e=kap/m3
    cperp2=max(0.0,a2-kap*kap/m3)
    physical_residual2=(nu_e-2.0*nu)**2*m3+cperp2
    norm_difference=a2-physical_residual2
    expected_difference=4.0*nu*m3*(nu_e-nu)
    scale=max(1.0,abs(norm_difference),abs(expected_difference))
    if abs(norm_difference-expected_difference)>2.0e-11*scale:
        raise AssertionError("orthogonal pair road failed to cancel from critical square")
    critical_rate=norm_difference/(2.0*nu)
    expected_rate=2.0*m3*(nu_e-nu)
    if abs(critical_rate-expected_rate)>2.0e-11*max(1.0,abs(critical_rate),abs(expected_rate)):
        raise AssertionError("endogenous Euler coefficient lost critical balance")
    alignment=0.0 if a2==0.0 else kap*kap/(a2*m3)
    return {
        "endogenous_euler_coefficient":nu_e,
        "signed_productive_reynolds":nu_e/nu,
        "productive_projection_squared":nu_e*nu_e*m3,
        "orthogonal_reconfiguration_squared":cperp2,
        "pair_area_squared":a2,
        "physical_residual_squared":physical_residual2,
        "critical_square_norm_difference":norm_difference,
        "critical_rate":critical_rate,
