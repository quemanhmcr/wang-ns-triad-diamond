...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_regeneration_persistence": "the historical nu_E=kappa/M3 reading remains exact, and the old necessity int kappa^2/[N^2(EZ-K^2)] dt=infinity is exactly int a^2/N^2 dt=infinity and the Poisson-boundary action Escript|R'_E|^2/[Escript_yy(Escript Escript_yy-Escript_y^2)]; the remaining theorem is finite energy-paid history for the intrinsic Krylov turning/reconfiguration law without losing the extra |C| weight",
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language now bottoms out before any triad split at u_t=J_u r-nu C^2u together with delta Theta=a h+R_w: one-step productivity has no sign monotone, nonzero h rotates the productive frame, helicity may force side motion, and suppressing both approaches two-radius rigidity; the unproved theorem is derivative-preserving historical control of int a^2/N^2 dt by physical heat",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel, infinite productive regeneration and quantitative stability/history readings are now a Krylov-turning/reconfiguration problem: A_escape=a^2/N^2 and delta Theta=a h+R_w exactly, but h has no proved uniform lower gap, h=0 is the two-radius rigid face and R_w carries an extra |C| weight; Cartan/Jacobi/Nijenhuis positive composition remains a network reading, while the missing theorem is finite energy-paid history for this intrinsic turning/side-motion/near-two-radius trichotomy",
