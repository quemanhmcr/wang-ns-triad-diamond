...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_one_step_turning_guard": "one fixed physical Fourier triangle/helicity assignment realizes all four sign quadrants of (kappa,kappa'_E) by changing only complex amplitudes, and normalized signed productivity can be positive and increasing; there is no one-step self-turning theorem",
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language bottoms out at fixed two-road dynamics u_t=J_u r-nu C^2u: one-step productive orientation has no sign monotone; actual continuation carries signed-curl gaps, bracket compositions obey weighted Jacobi, Fourier diamonds constrain carrier locations and M^2 F_log<=Q_triangle makes near-lossless steps nearly stationary; the unproved theorem is a representation-free positive composition law converting genuine repeated log displacement into heat-visible cost without gross triad traffic",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel, infinite productive regeneration and quantitative stability/history readings are now a positive-composition problem: kappa may be productive and keep increasing for one step; weighted Jacobi constrains bracket compositions but full Euler companions can vanish after signed-curl gaps, while nontrivial scale progress is paid occurrence-wise by Nijenhuis curvature; what remains unproved is a representation-free noncancelling composition theorem strong enough to make normalized escape action integrable",
