...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language now bottoms out at u_t=J_u r-nu C^2u with r=(C-H/E)u: J_u r is the signed two-way skew orientation current, -nu C^2u the one-way heat, and kappa=<(Lambda-K/E)u,J_u r>; q_t+partial_y(j_E+(nu/2)q_y)=0 is an exact scalar ledger but phase reversal proves q is not orientation-complete, so the unproved theorem is orientation-persistence/history rather than scalar closure",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel, infinite productive regeneration and quantitative stability/history readings all require the orientation retained by the skew Euler current J_u r; the scalar q/j ledger is exact but not orientation-complete and its transport action does not dominate escape action, so the remaining unproved step is a finite-time orientation-persistence theorem strong enough to make the exact normalized productive action integrable",
