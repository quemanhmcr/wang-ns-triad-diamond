...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language collapses to q_t+partial_y(j_E+(nu/2)q_y)=0: one completely monotone q contains the Sobolev ladder, its tail is the same critical stock at every canonical depth, j_+^E(0)=j_-^E(0)=kappa, Euler current has zero depth integral, heat current has one sign, and escape action is the normalized squared boundary Euler flux over the Hankel spread defect; the unproved theorem is no infinitely thin physical Euler return near y=0",
        ...
        "primitive_turning_frontier": "the historical persistent critical near-kernel, infinite productive regeneration and quantitative stability/history readings now all constrain the scalar physical current j_E in q_t+partial_y(j_E+(nu/2)q_y)=0; the remaining unproved step is a finite-time no-infinitely-thin-return law strong enough to make the exact normalized boundary-flux escape action integrable",
