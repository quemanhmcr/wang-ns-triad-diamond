...
def theorem_certificate() -> dict[str, object]:
    return {
        ...
        "primitive_poisson_transverse_activity": "A_y=int v^T[(tr tau_y)I-tau_y]v>=0 gives |R_E(y)|^2<=4 A_y(Z-Z_y); on continuum R3 A'_0=2||A_u||_pair^2 and the y->0 limit is kappa^2<=||A_u||_pair^2 M3; Germano gives A_(y+z)=A_z(P_yu)+a second nonnegative inherited-covariance term, but not monotonicity; the missing theorem is a finite-history boundary-stress estimate on A_y from the physical cocycle/heat/divergence laws, not an L-infinity shortcut",
        ...
        "primitive_two_road_frontier": "the historical successfully regenerated/accumulated infinitely often language now has the lower covariance-stress form: U+V=E, Euler exchanges U<->V through tau_y, viscosity removes from both, and D_y is the stress divergence with Fisher cost paid by Z-Z_y; the remaining unproved theorem is a finite-history bound on transverse activity A_y strong enough to force finite normalized boundary action",
